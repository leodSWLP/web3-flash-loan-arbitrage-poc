...
const callFlashSwap = async () => {
  try {
    ...
    const [data, blockNumber] = await Promise.all([
      ShareContentLocalStore.getStore().viemChainClient.readContract({
    ...
    console.log(
      `${new Date().toLocaleString('en-US', {
        ...
        hour12: true,
      })}: blockNumber form ZAN: ${blockNumber}, Read Data:`,
      data,
    );
  } catch (error) {
...
const TraceDelay = () => {
  createPublicClient({
    chain: bsc,
    transport: webSocket('wss://api.zan.top/node/ws/v1/bsc/mainnet/0ef6d89279034303b4eef3984df0f249'),
    
  }).watchBlockNumber({
    onBlockNumber: async (blockNumber) => {
      // Get current time with millisecond precision
      const now = new Date();
      const timestamp = now.toISOString() + '.' + now.getMilliseconds().toString().padStart(3, '0');
      
      
      try {
        const fetchTimestamp = new Date();
        const fetchedBlockNumber = await ShareContentLocalStore.getStore().viemChainClient.getBlockNumber();
        const fetchTimestampStr = fetchTimestamp.toISOString() + '.' + fetchTimestamp.getMilliseconds().toString().padStart(3, '0');
        
        console.log(`New block detected: ${blockNumber}, Fetched block number: ${fetchedBlockNumber} at ${fetchTimestampStr}`);
        
        // Compare block numbers and calculate time difference
        if (fetchedBlockNumber === blockNumber) {
          const timeDiffMs = fetchTimestamp.getTime() - now.getTime();
          console.log(`Block numbers match. Time difference: ${timeDiffMs}ms`);
        } else {
          console.log(`Block number mismatch! watchBlockNumber: ${blockNumber}, getBlockNumber: ${fetchedBlockNumber}`);
        }
      } catch (error) {
...
const viemChainClient = createPublicClient({
  chain: bsc,
  transport: http(ConfigUtil.getConfig().BSC_RPC_URL, { timeout: 120_000 }),
});
